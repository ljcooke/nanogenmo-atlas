...
class Exoplanets(XmlCorpus):
    ...
    def print_stats(self):
        ...
        print('')
        print('Total Systems:       %12d' % total_systems)
        print('Total Stars:         %12d' % total_stars)
        print('Total Planets:       %12d' % total_planets)
        print('')
        print('Minimum Stars:       %12d' % min(star_counts))
        print('Maximum Stars:       %12d' % max(star_counts))
        print('Average Stars:       %17.4f' % (float(total_stars) / len(star_counts)))
        print('')
        print('Minimum Planets:     %12d' % min(planet_counts))
        print('Maximum Planets:     %12d' % max(planet_counts))
        print('Average Planets:     %17.4f' % (float(total_planets) / len(planet_counts)))
        print('')
        print('Earliest Settled:    %12d' % min(years_settled))
        print('Latest Settled:      %12d' % max(years_settled))
